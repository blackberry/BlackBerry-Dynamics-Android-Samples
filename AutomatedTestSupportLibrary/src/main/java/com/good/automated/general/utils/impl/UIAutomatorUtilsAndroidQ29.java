...
public class UIAutomatorUtilsAndroidQ29 extends AbstractUIAutomatorUtils {
    ...
    public boolean forceStopApp(String applicationId) {
        ...
        if (forceStopButton.waitForExists(of(WAIT_FOR_SCREEN))) {
            Log.d(TAG, packageAndroidSettings + _ID + "button3 was found on system UI");
        } else {
            //Possible timing issue in opening system Setting UI
            waitForUI(of(UI_WAIT));
            Log.d(TAG, "Second attempt to find " + packageAndroidSettings + _ID + "button1_negative on system UI");
            forceStopButton = findByResourceId(packageAndroidSettings + _ID + "button1_negative");
        }
        try {
            if (forceStopButton.getText().toLowerCase().contains(textForceStop)) {
                //This is classic placing of Force stop button on system UI
                return performForceStopAction("button3");
            } else {
                Log.d(TAG, "button3 were not found on system UI");
                return false;
            }
        } catch (UiObjectNotFoundException e) {
            Log.d(TAG, "button3 were not found on system UI");
