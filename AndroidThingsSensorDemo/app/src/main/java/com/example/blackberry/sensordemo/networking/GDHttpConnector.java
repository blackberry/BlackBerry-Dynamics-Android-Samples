...
public class GDHttpConnector
{
    ...
    public String doRequest(HttpRequestParams params) throws IOException
    {
        ...
        try {
            ...
            switch (params.getRequestType()) {
                case HttpRequestParams.POST:
                    stream = makePostRequest(params);
                    break;
                case HttpRequestParams.GET:
                    stream = makeGetRequest(params);
                    break;
                case HttpRequestParams.PUT:
                    stream = makePutRequest(params);
            }
     ...
     * an input stream.
     */
    private InputStream makePutRequest(HttpRequestParams params) throws IOException
    {

        GDHttpClient httpclient = new GDHttpClient();
        final HttpPut request = new HttpPut(params.getUrl());
