...
#include <jni.h>
#include <pthread.h>
/* Blackberry modifications - Use GD C File API not standard C File API  */
#include <GD_C_filesystem.h>
//#include <stdio.h>
        ...
        XAuint32 itemsLength           /* input */)
{
    ...
    if (discontinuity) {
        ...
        if (!reachedEof) {
            ...
            /* Blackberry modifications - use GD Secure Container  */
            GD_rewind(file);
            /* Blackberry modifications - end  */
    ...
    /* Blackberry modifications - use GD Secure Container  */
    bytesRead = GD_fread(pBufferData, 1, BUFFER_SIZE, file);
    LOGV("Number Bytes read = %d ", bytesRead);
    /* Blackberry modifications - end  */
...
static jboolean enqueueInitialBuffers(jboolean discontinuity)
{
    ...
    /* Blackberry modifications - use GD Secure Container  */
    bytesRead = GD_fread(dataCache, 1, BUFFER_SIZE * NB_BUFFERS, file);
    LOGV("GD read, bytes read = %d", bytesRead);
    /* Blackberry modifications - end  */
        ...
        jclass clazz, jstring filename)
{
    ...
    /* Blackberry modifications - use GD Secure Container  */
    file = GD_fopen(utf8, "rb");
    LOGV("GD open file name = %s", utf8);
    /* Blackberry modifications - end  */
    
    if (file == NULL) {
        LOGV("open file is NULL");
        return JNI_FALSE;
    }
